...
export function* onFetchAgents(): Iterable<any> {
  ...
  const data: types.AgentsApiResponse = yield call(
    callFetchAgentsWithTimeout,
    host,
    auth,
    period
  );
...
export function* onToggleAlerts(): Iterable<any> {
  const host = yield select(userSelectors.getAPIServerURL);
  const data: types.AlertsApiResponse = yield call(callToggleAlerts, host);
...
export function* onFetchAlerts(): Iterable<any> {
  ...
  const data: types.AlertsApiResponse = yield call(callAlerts, host, auth);
...
export function* onToggleType2({
  ...
  const host = yield select(userSelectors.getAPIServerURL);
  const data: types.AlertsApiResponse = yield call(
    callToggleType2,
    host,
    agentID
  );
...
function callRemoveAgent(host, auth, agentID) {
  ...
  return fetch(request)
    ...
    .catch(() => `Removing agent ${agentID} request failed`);
}

export function* onRemoveAgent(action: { agentID: string }): Iterable<any> {
